...
class EditAccount extends React.Component {
    ...
    state = {
      ...
      componentDidMount(){
          this.setState({ email: this.props.user.email, password:  this.props.user.password, city: this.props.user.city, gender: this.props.user.gender })
      }
  
      render () {
    
    const { handleChange, handleSubmit } = this
    const { email, password, city, gender, showErrors } = this.state
    return (
        < >
            ...
            e.preventDefault()
            handleSubmit(email, password, city, gender)
            }} > 
            Submit
            </button>
            </form>
